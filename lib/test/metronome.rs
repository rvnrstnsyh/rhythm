...
mod metronome_constants {
    use lib::metronome::{
        DEFAULT_BATCH_SIZE, DEFAULT_CHANNEL_CAPACITY, DEFAULT_DEV_PHASES_PER_CYCLE, DEFAULT_HASH, DEFAULT_HASHES_PER_REV, DEFAULT_HASHES_PER_SECOND,
        DEFAULT_MS_PER_PHASE, DEFAULT_MS_PER_REV, DEFAULT_NUM_CONSECUTIVE_LEADER_PHASES, DEFAULT_PHASES_PER_CYCLE, DEFAULT_REVS_PER_DAY, DEFAULT_REVS_PER_PHASE,
        DEFAULT_REVS_PER_SECOND, DEFAULT_S_PER_PHASE, DEFAULT_SECONDS_PER_DAY, DEFAULT_SPINLOCK_THRESHOLD_US, DEFAULT_US_PER_REV, DEFAULT_US_TOLERANCE_PER_REV,
    };

    #[test]
    fn default_hash_is_zero() {
        // Testing mutable static requires a different approach that avoids creating shared references.
        let value: u8 = unsafe { core::ptr::addr_of!(DEFAULT_HASH).read_volatile() };
        assert_eq!(value, 0, "DEFAULT_HASH should be 0 (SHA-256).");
    }
